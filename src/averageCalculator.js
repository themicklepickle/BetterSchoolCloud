...
const cleanseValues = (markBooks) => {
  try {
    markBooks.forEach((book, i, bookArray) => {
      // Loop through each markbook
...
const addColumnAfter = (i, name, html) => {
  $(`#TableSecondaryClasses tr:first td:nth-child(${i}):first`).after(
    `<td class="mwTABLE_CELL_HEADER" align="center" rowspan="2">${name}</td>`
  );
  $(
    `#TableSecondaryClasses > tbody > tr > td:not(.mwTABLE_CELL_HEADER):nth-child(${i})`
  ).each(function () {
...
const addColumnBefore = (i, name, html) => {
  $(`#TableSecondaryClasses tr:first td:nth-child(${i}):first`).before(
    `<td class="mwTABLE_CELL_HEADER" align="center" rowspan="2">${name}</td>`
  );
  $(
    `#TableSecondaryClasses > tbody > tr > td:not(.mwTABLE_CELL_HEADER):nth-child(${i})`
    ...
    if (localStorage.weights) {
      ...
      weights.forEach((weight) => {
        $("#TableSecondaryClasses tr").each(function (i, row) {
          ...
          if ($row.find("td:first").text() === weight.name) {
...
const fetchMarkbooks = async (markBooks, cleanedMarkbooks) => {
  try {
    ...
    const postURL =
      currentURL.origin +
      currentURL.pathname +
      "/../../../viewer/Achieve/TopicBas/StuMrks.aspx/GetMarkbook"; // Segment and add the parts to a single string
    markBooks.forEach((markbook) => {
...
const addItemToTable = (item, itemName, id) => {
   ...
   */
  const tableClass =
    $("#TableSecondaryClasses tr:last > td").attr("class").substr(13, 1) === "A"
      ? "B"
      : "A";

  /* Appends the row to the end of the table */
  $("#TableSecondaryClasses tr:last").after(
    "<tr><td class='mwTABLE_CELL_" +
      tableClass +
      "'>" +
      itemName +
      "</td><td class='mwTABLE_CELL_" +
      tableClass +
      "' id='" +
      id +
      "'>" +
...
const injectScores = async () => {
  try {
    ...
    ); // Add the column to hold the marks
    addColumnAfter(
      2,
      "Weight",
    ...
    $("#TableSecondaryClasses table").prepend(
...
const pollScores = () => {
  if ($("#TableSecondaryClasses tr:last > td:first").text() !== "Average") {
    // Checks if the last row is not correct
