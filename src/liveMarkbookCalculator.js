...
const calculateLayer = (layer) => {
  ...
  for (let i = 0; i < layer.length; i++) {
    ...
    if (isNaN(mark) || isNaN(weight) || isNaN(markDenom))
      // EXC, ABS, and blanks are ignored
      continue;

    if (mark < 0)
      // Treat negative marks as 0
      mark = 0;
    if (weight < 0)
      // Treat negative weights as 0
      weight = 0;
    if (markDenom <= 0)
      // If the denominator does not make sense, ignore whole grade
      continue;
    denominator += weight;
...
const calculateMarks = () => {
  ...
  for (let i = 0; i < markbook.length; i++) {
    ...
    if (
      (isNaN(parseFloat(markbook[i].mark)) && markbook[i].mark !== "") ||
      markbook[i].row.find(selectors.mark).is(":hidden")
    )
      // Make sure top isn't already invalid or hidden
      continue;

    if (middle && middle.length) {
      // Make sure there is a middle layer to handle
      for (let j = 0; j < middle.length; j++) {
        if (
          (isNaN(parseFloat(middle[j].mark)) && middle[j].mark !== "") ||
          middle[j].row.find(selectors.mark).is(":hidden")
        )
          // Make sure middle isn't already invalid or hidden
          continue;
      ...
      markbook[i].mark =
        calculateLayer(markbook[i].children) * markbook[i].denominator; // Set the new mark
    }
  }
...
const highlightChanges = () => {
  $("#markbookTable table tbody > tr:gt(0)").each(function (i) {
    ...
    if (currentDenominator.val() !== initialMarkbook[i].denominator.val) {
      ...
      currentDenominator.css(
        "background-color",
        initialMarkbook[i].denominator.bgColor
      );
    }
  });
};
